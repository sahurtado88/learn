...
module "sa_queue_pep" {
  ...
  pep_name             = local.sa_queue_pep_name
  ...
  subresource_names    = var.sa_queue_subresource_names
  private_zone_id      = data.azurerm_private_dns_zone.sa_queue_private_dns[count.index].id
  depends_on = [
...
module "sa_queue_pep_internal" {
  ...
  pep_name             = join("-", [local.sa_queue_pep_name, var.internal_name])
  pep_manual_conn      = var.sa_gl_pep_manual_conn
  private_conn_name    = module.global_storage_account.storage_name
  private_conn_id      = module.global_storage_account.storage_id
  ...
  subresource_names    = var.sa_queue_subresource_names
  private_zone_id      = data.azurerm_private_dns_zone.sa_queue_internal_private_dns[count.index].id
  depends_on = [
    module.sa_queue_pep # Because parallel PEP creation fails
  ]
}