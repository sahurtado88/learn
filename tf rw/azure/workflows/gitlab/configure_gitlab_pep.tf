module "postgres_pep" {
  ...
  pep_name             = local.postgres_pep_name
  pep_manual_conn      = var.postgres_pep_manual_conn
  private_conn_name    = module.postgres_gitlab[0].name
  private_conn_id      = module.postgres_gitlab[0].id
  network_rg_name      = local.rg_network
  pep_vnet_name        = data.azurerm_virtual_network.common_vnet.name
  subresource_names    = var.postgres_subresource_names
  private_zone_id      = module.postgres_private_dns_zone[count.index].id
  depends_on = [
...
module "sa_gitlab_pep" {
  ...
  depends_on = [
    #module.global_storage_account.name, Verify with Anderson
    module.redis_pep_internal # Because parallel PEP creation fails
  ]
}
data "azurerm_private_dns_zone" "sa_queue_private_dns" {
  count               = var.gitlab_manage && var.pep_manage ? 1 : 0
...
module "postgres_pep_internal" {
  ...
  rg_name              = var.internal_pep_rg_name
  subnet_endpoint_name = var.internal_pep_subnet_name
  pep_name             = join("-", [local.postgres_pep_name, var.internal_name])
  pep_manual_conn      = var.postgres_pep_manual_conn
  private_conn_name    = module.postgres_gitlab[0].name
  private_conn_id      = module.postgres_gitlab[0].id
  network_rg_name      = var.internal_network_rg_name
  pep_vnet_name        = var.internal_pep_vnet_name
  subresource_names    = var.postgres_subresource_names
  private_zone_id      = data.azurerm_private_dns_zone.postgres_internal_private_dns[count.index].id
  depends_on = [
    module.postgres_pep # Because parallel PEP creation fails
  ]
}
