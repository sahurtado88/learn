...
module "app-gtwy" {
  ...
  app_gateway_frontend_subnet_address_prefix = "10.${var.env_index * (floor((var.env_max_clusters - 1) / 20) + 1) * 10}.64.0/25"
  ...
  app_gateway_default_listener_name           = "portal"
  ...
  depends_on = [
    azurerm_key_vault_certificate.domaincert
  ]
