...
resource "kubernetes_storage_class" "ra-azureblob" {
  ...
  metadata {
    name = join("-", ["ra-blob", var.database_secret_namespace_project])
    labels = {
  ...
  parameters = {
    skuName                                           = var.common_storage_replication_type
    "csi.storage.k8s.io/provisioner-secret-name"      = data.azurerm_storage_account.monitoring_storage_account.name
    "csi.storage.k8s.io/provisioner-secret-namespace" = var.database_secret_namespace_project
    "csi.storage.k8s.io/node-stage-secret-name"       = data.azurerm_storage_account.monitoring_storage_account.name
    "csi.storage.k8s.io/node-stage-secret-namespace"  = var.database_secret_namespace_project
  }
}
