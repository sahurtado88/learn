...
module "app-gtwy" {
  ...
  app_gateway_frontend_name                    = format(local.app_gateway_frontend_name, var.cluster_index)
  ...
  app_gateway_frontend_subnet_address_prefix = "10.${floor(var.cluster_index / 2) + (var.env_index * (floor((var.env_max_clusters - 1) / 20) + 1) * 10)}.${var.cluster_index % 2 == 0 ? 64 : 192}.0/25"

  app_gateway_ip_configuration            = var.app_gateway_ip_configuration
  app_gateway_name                        = format(local.app_gateway_name, var.cluster_index)
  app_gateway_public_ip_allocation_method = var.app_gateway_public_ip_allocation_method
  # app_gateway_public_ip_name                  = format(local.app_gateway_public_ip_name, var.cluster_index)
  app_gateway_rg_name                         = local.rg_application_gateway
  ...
  app_gateway_default_listener_name           = format("${var.env_prefix}${var.env_name}%03d", var.cluster_index)
  ...
  app_gateway_vnet_name                       = format(local.app_gateway_vnet_name, (floor(var.cluster_index / 4)))
